...
当前仓库可由 `0001` 到 `0007` 的 `down_revision` 链推导最终结构：
...
增加消费周期；`0007` 增加购买条件。本文不声称远程数据库当前版本，除非另有现场
...
| `user_fruit_preferences` | `user_id`、`fruit_id`、`preference_score`、`is_forbidden`、`has_tried`、`willing_to_try` | user/fruit 联合唯一；用户删除时级联偏好；`NULL` 与 `False` 有不同语义 |
...
只读查看 `0001` 与 `0007`：找出 `recommendations` 的两个“防重复”约束，并说明它们
分别防止什么。不要执行 migration。
...
教学时应同时阅读 migration 和 ORM，不能只看最新 migration 或只看 Python class。`n