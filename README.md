...
| 已参与推荐 | 价格、甜/酸/质地、食用便利、尝鲜程度、喜欢/不喜欢/禁止、是否吃过、是否愿意尝试、带证据的采收季、历史和反馈 |
| 当前保持中性 | 地区只用于匹配消费者市场可得性证据；当前 24 条市场记录均因缺少可靠地区库存来源而停用，所以地区暂不改变排序 |
...
`season_score` 只来自启用且带来源的国内产地采收窗口，不再按用户所在地区
加分；`availability_score` 只表示消费者市场可得性。缺少可验证市场证据时使用
0.45 中性值，既不生成“本地容易购买”理由，也不把未知库存写成 `available`。
当前外层和内层权重未因本次语义修复而改变。
...
| `data/fruit_harvest_windows.csv` | 国内生产区域、采收月份、离散季节档位、来源年份和证据质量 |
| `data/fruit_market_availability.csv` | 消费者地区市场可得性；缺少库存/监测证据的记录保持 `unknown` 且停用评分 |

`default_portion_grams` 是建议份量元数据，当前不参与营养计算。算法直接对
0～1 的无物理单位演示分数做 P05/P95 归一化；这些数值不代表真实的克或毫克，
也不构成每份营养计算。

项目中的采收月份仍是月级粗粒度资料，不代表实时库存、进口供应或用户附近商店一定可买到。价格和部分营养数据用于软件功能演示，不构成医学或专业营养建议。营养字段是**归一化演示分数**，不表示每 100 克的真实克数或毫克数。

季节证据按可复核规则分级：`high` 表示政府或农业主管部门的一手资料直接覆盖同一水果、生产地区和启用月份；`medium` 表示官方农业新闻或产区报道明确覆盖该窗口，但粒度较粗；`low` 与 `unverified` 只可保留作待核资料，不得启用评分。启用行还必须保存来源年份和可定位的来源说明。市场记录若同月同地区发生冲突，算法先采用证据等级更高者；同等级冲突采用更保守的 `unavailable > unknown > available` 状态，并使用完整稳定键消除输入顺序影响。
...
seed 写入器会同时读取 `fruits_seed.json`、`fruit_profile_seed.json`、冷知识和消费类型 seed；后者是甜、酸、质地、便利和适食后常温保存等人工标注的单一配置源。重复运行使用幂等 upsert；季节行带受管来源前缀，写入前仅停用已不在当前 CSV 的受管窗口，保留行供审计且不触碰人工维护记录。默认写入只允许本地可丢弃的 `daily_fruit_test`。向已确认的 Supabase 迁移库写入时，必须显式设置 `MIGRATION_DATABASE_URL`、`SUPABASE_URL`、`DAILY_FRUIT_ALLOW_MIGRATION_SEED=yes` 并传入 `--migration`；数据库 project ref 必须与 Supabase origin 一致。连接串只能来自本地环境或部署 Secret。
...
- 仅有 24 种演示水果，水果口感、便利性、常见度和价格的部分值是人工标注；采收季虽记录来源，仍只有月份与大区粒度。
- 营养值为 0～1 的无物理单位演示分数；`default_portion_grams` 当前不参与营养计算。
- 尚未接入消费者地区库存或市场监测数据，因此市场可得性保持中性未知；价格也不是实时市场数据。
