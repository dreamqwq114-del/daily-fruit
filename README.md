...
`default_portion_grams` 是建议份量元数据。当前算法会先按份量比例处理营养值，再做 P05/P95 归一化；但源 CSV 本身是无物理单位的归一化演示分数，因此该份量换算没有真实的克或毫克含义，并会产生额外的份量权重。这是已知的数据语义技术债，不应解释为真实每份营养计算。
